...
const CenterBannersPage = () => {
  ...
  const handleUpload = async (file: File) => {
    ...
    const path = `centers/${primaryCenterId}/${Date.now()}.${ext}`;
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black font-display text-foreground">Centre Page Banners</h1>
          <p className="text-sm text-muted-foreground">Hero & promo banners shown on your centre's public page.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div key={b.id} className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => setEditing(b)} className="rounded-md border border-border px-3 py-1 text-xs">Edit</button>
                <button onClick={() => handleDelete(b.id)} className="rounded-md border border-destructive/40 text-destructive px-3 py-1 text-xs inline-flex items-center gap-1">
                  <Trash2 className="h-3 w-3" /> Delete
                </button>
                <span className={`ml-auto rounded-full px-2 py-1 text-[10px] font-bold ${b.is_active ? "bg-secondary/10 text-secondary" : "bg-muted text-muted-foreground"}`}>
        ...
        {!items.length && <p className="text-sm text-muted-foreground">No banners yet. Add your first one.</p>}
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-lg rounded-xl bg-card p-6 space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            <input value={editing.title ?? ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} placeholder="Title" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            <input value={editing.subtitle ?? ""} onChange={(e) => setEditing({ ...editing, subtitle: e.target.value })} placeholder="Subtitle" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            <div className="grid grid-cols-2 gap-2">
              <input value={editing.cta_label ?? ""} onChange={(e) => setEditing({ ...editing, cta_label: e.target.value })} placeholder="CTA label" className="rounded-md border border-border bg-background px-3 py-2 text-sm" />
              <input value={editing.cta_url ?? ""} onChange={(e) => setEditing({ ...editing, cta_url: e.target.value })} placeholder="CTA URL" className="rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-2">
...
export default CenterBannersPage;
