...
const CenterTestSeriesPage = () => {
  ...
  const canCreate = isAdmin || can("test_series", "create");
  const canEdit = isAdmin || can("test_series", "edit");
  const canDelete = isAdmin || can("test_series", "delete");
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("test_series")
      .select("id, slug, title, target_exam, total_tests, price, original_price, is_published, is_featured, thumbnail_url, centre_id")
      ...
      <div className="rounded-2xl bg-[#1C3F8E] p-6 text-white flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/90 text-sm mt-1">Manage your centre's test series, alongside Bansal's global series</p>
          </div>
        </div>
        {canCreate && (
          <Link
            to="/center/test-series/new"
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => {
                  const isOwn = r.centre_id === primaryCenterId;
                  const rowCanEdit = canEdit && isOwn;
                  const rowCanDelete = canDelete && isOwn;
                  return (
                  <tr key={r.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => rowCanEdit && togglePublish(r)}
                        disabled={!rowCanEdit}
                        className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${r.is_published ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"} ${!rowCanEdit ? "cursor-default" : ""}`}
                      >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {rowCanEdit && (
                          <button
                            onClick={() => navigate(`/center/test-series/${r.id}/edit`)}
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={9} className="px-4 py-10 text-center text-muted-foreground">
                      No test series yet. {canCreate ? 'Click "New Test Series" to add one.' : ""}
                    </td>
                  </tr>
                )}
              </tbody>
