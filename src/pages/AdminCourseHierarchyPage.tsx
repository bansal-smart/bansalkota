...
import { ArrowLeft, Plus, Trash2, Pencil, ChevronRight, ChevronDown, GripVertical, Eye, Video, FileText, ClipboardList, ExternalLink } from "lucide-react";
...
function QuizTab({ subtopic, courseId, onSaved }: { subtopic: CourseSubtopic; courseId: string; onSaved: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      if (subtopic.quiz?.id) {
        const { data } = await supabase.from("subtopic_quiz_questions" as any).select("*").eq("quiz_id", subtopic.quiz.id).order("position");
        setQuestions((data ?? []) as any);
      } else setQuestions([]);
    })();
  }, [subtopic.id, subtopic.quiz?.id]);

  const createQuiz = async () => {
    const { error } = await supabase.from("subtopic_quizzes" as any).insert({
      course_id: courseId, subtopic_id: subtopic.id, ...meta, time_limit_minutes: meta.time_limit_minutes || null,
    });
    if (error) return toast.error(error.message);
    setOpen(false); toast.success("Quiz created"); onSaved();
  };
  ...
  const addQuestion = async () => {
    if (!subtopic.quiz?.id) return;
    const { data, error } = await supabase.from("subtopic_quiz_questions" as any).insert({
      quiz_id: subtopic.quiz.id, question_text: "New question", option_a: "A", option_b: "B", option_c: "C", option_d: "D",
      correct_option: "a", marks: 4, negative_marks: 1, position: questions.length,
    }).select().single();
    if (error) return toast.error(error.message);
    setQuestions([...questions, data as any]);
  };
  const updateQ = async (q: SubtopicQuizQuestion) => {
    const { id, ...rest } = q;
    await supabase.from("subtopic_quiz_questions" as any).update(rest).eq("id", id);
  };
  const delQ = async (id: string) => {
    await supabase.from("subtopic_quiz_questions" as any).delete().eq("id", id);
  ...
  if (!subtopic.quiz) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted-foreground mb-3">No quiz for this subtopic.</p>
        <Button onClick={() => setOpen(true)}><Plus className="h-3 w-3 mr-1" /> Create Quiz</Button>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Time limit (min, 0 = none)</Label><Input type="number" value={meta.time_limit_minutes} onChange={(e) => setMeta({ ...meta, time_limit_minutes: Number(e.target.value) })} /></div>
                <div><Label>Pass %</Label><Input type="number" value={meta.pass_percentage} onChange={(e) => setMeta({ ...meta, pass_percentage: Number(e.target.value) })} /></div>
              </div>
            </div>
            <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={createQuiz}>Create</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
    );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between p-3 border rounded">
        <div>
          <div className="font-medium">{subtopic.quiz.title}</div>
          <div className="text-xs text-muted-foreground">{questions.length} Q · {subtopic.quiz.time_limit_minutes ?? "No"} min · Pass {subtopic.quiz.pass_percentage}%</div>
        </div>
        <div className="flex gap-2">
          <Button size="sm" onClick={addQuestion}><Plus className="h-3 w-3 mr-1" /> Add Question</Button>
          <Button size="sm" variant="outline" onClick={deleteQuiz}><Trash2 className="h-3 w-3" /></Button>
        </div>
      </div>
      <div className="space-y-3">
        {questions.map((q, i) => (
          <QuestionRow key={q.id} q={q} index={i} onChange={(nq) => { setQuestions(questions.map((x) => x.id === nq.id ? nq : x)); updateQ(nq); }} onDelete={() => delQ(q.id)} />
        ))}
      </div>
    </div>
  );
...
function QuestionRow({ q, index, onChange, onDelete }: { q: SubtopicQuizQuestion; index: number; onChange: (q: SubtopicQuizQuestion) => void; onDelete: () => void }) {
  const [local, setLocal] = useState(q);
  useEffect(() => { setLocal(q); }, [q.id]);
  const change = (patch: Partial<SubtopicQuizQuestion>) => { const next = { ...local, ...patch }; setLocal(next); onChange(next); };
  return (
    <div className="border rounded p-3 space-y-2">
      ...
      <Textarea value={local.question_text} onChange={(e) => change({ question_text: e.target.value })} rows={2} placeholder="Question text" />
      <div className="grid grid-cols-2 gap-2">
        {(["a", "b", "c", "d"] as const).map((k) => (
          <div key={k} className="flex items-center gap-2">
            <input type="radio" checked={local.correct_option === k} onChange={() => change({ correct_option: k })} />
            <Input value={(local as any)[`option_${k}`]} onChange={(e) => change({ [`option_${k}`]: e.target.value } as any)} placeholder={`Option ${k.toUpperCase()}`} />
          </div>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-2">
        <div><Label className="text-xs">Marks</Label><Input type="number" value={local.marks} onChange={(e) => change({ marks: Number(e.target.value) })} /></div>
        <div><Label className="text-xs">Negative</Label><Input type="number" step="0.1" value={local.negative_marks} onChange={(e) => change({ negative_marks: Number(e.target.value) })} /></div>
        <div className="col-span-1"><Label className="text-xs">Explanation</Label><Input value={local.explanation ?? ""} onChange={(e) => change({ explanation: e.target.value })} /></div>
      </div>
    </div>
  );
}
