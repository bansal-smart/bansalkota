...
// A centre's OWN gallery (centre_gallery table) — distinct from AdminGalleryPage,
// which manages the site-wide public gallery (gallery_albums). Reached from
// /admin/centre-gallery, only relevant to center_admin users (RLS scopes reads/
// writes to their own centre; admin/super_admin bypass but have no centre to
// default to here, so this page is centre_admin-only in practice).
...
const AdminCentreGalleryPage = () => {
  const { primaryCenterId, loading: loadingCentre } = useCenterAdmin();
  ...
  if (loadingCentre) return <div className="p-8 text-sm text-muted-foreground">Loading…</div>;
...
export default AdminCentreGalleryPage;
