...
-- 1. Paper grouping + scheme columns on tests
ALTER TABLE public.tests
  ADD COLUMN IF NOT EXISTS paper_group_id uuid,
  ADD COLUMN IF NOT EXISTS paper_label text,
  ADD COLUMN IF NOT EXISTS partial_marking_scheme text DEFAULT 'standard',
  ADD COLUMN IF NOT EXISTS section_instructions jsonb DEFAULT '[]'::jsonb;

CREATE INDEX IF NOT EXISTS idx_tests_paper_group ON public.tests(paper_group_id) WHERE paper_group_id IS NOT NULL;

-- 2. Question reports
CREATE TABLE IF NOT EXISTS public.test_question_reports (
...
GRANT SELECT, INSERT, UPDATE ON public.test_question_reports TO authenticated;
GRANT ALL ON public.test_question_reports TO service_role;

ALTER TABLE public.test_question_reports ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users insert their own question reports"
  ON public.test_question_reports FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users view their own question reports"
  ON public.test_question_reports FOR SELECT TO authenticated
  USING (
    user_id = auth.uid()
...
CREATE POLICY "Staff update question reports"
  ON public.test_question_reports FOR UPDATE TO authenticated
  USING (
...
CREATE INDEX IF NOT EXISTS idx_tqr_test ON public.test_question_reports(test_id);
CREATE INDEX IF NOT EXISTS idx_tqr_question ON public.test_question_reports(question_id);
CREATE INDEX IF NOT EXISTS idx_tqr_user ON public.test_question_reports(user_id);

CREATE TRIGGER trg_tqr_updated_at
  BEFORE UPDATE ON public.test_question_reports
...
BEGIN
  PERFORM public.notify_admins(
    'Question reported',
    'A student reported a question. Reason: ' || NEW.reason,
    'question_report',
    '/admin/tests'
  );
  RETURN NEW;
...
CREATE TRIGGER trg_notify_question_report
  AFTER INSERT ON public.test_question_reports
  FOR EACH ROW EXECUTE FUNCTION public.notify_question_report();

-- 3. Update submit_test_attempt to support JEE Advanced partial scheme for mcq-multi
...
BEGIN
  ...
  v_scheme := COALESCE(v_test.partial_marking_scheme, 'standard');
    ...
    ELSE
      ...
      CASE COALESCE(q.question_type, 'mcq-single')
        WHEN 'mcq-multi' THEN
          ...
          IF v_scheme = 'jee_advanced' THEN
            -- JEE Advanced: +4 all correct, -2 any wrong, otherwise +1 per correct chosen, 0 if none
            IF exact THEN
              is_correct := true; q_marks := q_max;
            ELSIF any_wrong THEN
              q_marks := COALESCE(NULLIF(q.marks_wrong, 0), -2);
            ELSE
              q_marks := LEAST(selected_count, q_max);
            END IF;
          ELSE
            IF exact THEN
              is_correct := true; q_marks := q_max;
