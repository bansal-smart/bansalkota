...
-- 1) Update submit_test_attempt to persist subject breakdown into metadata
...
-- 2) Backfill helper: recompute subject_data for one attempt and merge into metadata
CREATE OR REPLACE FUNCTION public._backfill_subject_metadata_for_attempt(_attempt_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  UPDATE public.test_attempts
  SET metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('subjects', subject_data)
  WHERE id = _attempt_id;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public._backfill_subject_metadata_for_attempt(uuid) FROM PUBLIC, anon, authenticated;

-- 3) One-shot backfill for all submitted attempts missing subject breakdown
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
    ...
    WHERE status IN ('submitted','auto_submitted')
      AND (metadata IS NULL OR metadata->'subjects' IS NULL OR jsonb_typeof(metadata->'subjects') <> 'object')
  LOOP
    PERFORM public._backfill_subject_metadata_for_attempt(r.id);
  END LOOP;
END $$;
