      ...
      CASE COALESCE(q.question_type, 'mcq-single')
        WHEN 'mcq-multi' THEN
          ...
          ELSIF any_wrong THEN
            -- JEE Advanced Section-2: any wrong pick = -1 (default), respect admin override exactly
            q_marks := COALESCE(q.marks_wrong, -1);
          ELSIF q.partial_marking THEN
            -- JEE Advanced partial: +1 per correctly picked (capped at q_max)
            q_marks := LEAST(correct_picked, q_max);
          ELSE
            -- Partial disabled: subset without wrong picks earns nothing
            q_marks := 0;
          END IF;
