...
ALTER TABLE public.test_attempts ADD COLUMN IF NOT EXISTS result jsonb;

-- Internal scoring helper (no auth check) used by both submission and backfill
...
-- Public-facing wrapper used by the student client
...
DECLARE
  v_user uuid;
BEGIN
  SELECT user_id INTO v_user FROM public.test_attempts WHERE id = _attempt_id;
  ...
  END IF;
  RETURN public.score_test_attempt(_attempt_id);
END;
$function$;

GRANT EXECUTE ON FUNCTION public.submit_test_attempt(uuid) TO authenticated;

-- Admin recompute helper (admin / super_admin only)
...
-- Backfill the broken attempts (status submitted/auto_submitted with score 0 and answers present)
DO $$
DECLARE r record;
BEGIN
  FOR r IN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Backfill failed for %: %', r.id, SQLERRM;
    END;
  END LOOP;
END $$;
