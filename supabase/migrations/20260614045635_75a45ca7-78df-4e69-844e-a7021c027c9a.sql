...
-- 1) submit_test_attempt: only the student can submit; percentile uses each user's LATEST submitted attempt
...
BEGIN
  ...
  -- Only the student themselves may submit. No admin/teacher bypass.
  ...
  -- Percentile: compare against each OTHER user's latest submitted attempt
  WITH latest AS (
...
-- 2) get_test_rank: rank against each user's LATEST submitted attempt
...
BEGIN
  ...
  -- For the requesting user, use this user's LATEST submitted attempt's score
  -- as the comparison score, so older attempts also report consistent rank.
  WITH self_latest AS (
...
-- 3) admin_test_result_sheet: pick each student's LATEST submitted attempt (not best)
...
-- 4) Recompute percentile on each user's latest submitted attempt per test, using "latest per user" basis
DO $$
DECLARE
  r RECORD;
  total_n int;
...
BEGIN
  FOR r IN
    SELECT DISTINCT ON (test_id, user_id) id, test_id, user_id, score
    FROM public.test_attempts
    WHERE status IN ('submitted','auto_submitted')
    ORDER BY test_id, user_id, submitted_at DESC NULLS LAST
  LOOP
    WITH latest AS (
      SELECT DISTINCT ON (user_id) user_id, score
      FROM public.test_attempts
    ...
    SELECT COUNT(*), COUNT(*) FILTER (WHERE score < r.score)
    INTO total_n, lower_n FROM latest;
    new_pct := CASE WHEN total_n = 0 THEN 100 ELSE ROUND(lower_n::numeric * 100.0 / total_n, 1) END;
    UPDATE public.test_attempts SET percentile = new_pct WHERE id = r.id;
  END LOOP;
END $$;
