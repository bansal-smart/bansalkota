...
ALTER TABLE public.test_questions
  ADD COLUMN IF NOT EXISTS marks_unanswered numeric NOT NULL DEFAULT 0,
...
REVOKE SELECT (numerical_answer) ON public.test_questions FROM anon, authenticated;
      ...
      CASE COALESCE(q.question_type, 'mcq-single')
        ...
        WHEN 'mcq-multi' THEN
          ...
          ELSIF q.partial_marking AND selected_count > 0 AND correct_count_arr > 0 THEN
            q_marks := ROUND( (COALESCE(q.marks_correct,4)::numeric * selected_count) / correct_count_arr, 2);
          ELSE
