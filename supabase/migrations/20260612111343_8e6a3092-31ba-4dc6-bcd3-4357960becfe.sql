      ...
      CASE COALESCE(q.question_type, 'mcq-single')
        ...
        WHEN 'mcq-multi' THEN
          ...
          ELSIF any_wrong THEN
            -- Any wrong selection => full negative (JEE Advanced rule)
            q_marks := COALESCE(q.marks_wrong, -2);
          ELSIF q.partial_marking AND selected_count > 0 AND correct_count_arr > 0 THEN
            -- JEE Advanced partial: +1 per correctly selected option,
            -- capped at full marks. Per-correct unit derived as
            -- floor(marks_correct / correct_count_arr) so 4-mark/4-correct
            -- questions yield exactly +1 per option.
          ...
          ELSE
            -- Subset of correct answers without partial marking enabled => 0
            q_marks := 0;
          END IF;
